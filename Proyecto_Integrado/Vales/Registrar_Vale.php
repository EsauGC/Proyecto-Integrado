...
<form class="vent"  action="guardar_vale.php" method="POST" onsubmit="return validarVale()">
        <table>
        <thead>
            <tr>
                ...
                <th class="PCI_Venta">Coordinación</th>
                <th class="PCI_Venta">Recibio</th>
                <th class="PCI_Venta">Evidencia</th>
   ...
   <script>
    ...
    if (numProductos == 0) {
        alert("Por favor, añade al menos un producto antes de registrar el vale.");
...
$("#buscar").autocomplete({
    source: function (request, response) {
        $.ajax({
            url: "ObtenerProductosVL.php",  // PHP backend script to get products
            dataType: "json",
            data: { nombre: request.term },
            success: function (data) {
                response($.map(data, function (item) {
                    return {
                        label: item.nombre,  // Lo que se muestra en la lista de autocompletado
                        value: item.nombre,  // Valor que se coloca en el input al seleccionar
...
$("#buscar").on("autocompleteclose", function () {
    document.getElementById("buscar").value = "";
});
...
function agregarProducto(arrayInfoProducto) {
    ...
    // Obtener valores de la primera fila (si existe)
    let coordinacionInicial = document.getElementById("coordinacion_1")?.value || "";
    let recibioInicial = document.getElementById("recibio_1")?.value || "";
    let evidenciaInicial = document.getElementById("evidencia_1")?.value || "";
    ...
    htmlProducto += '<td><input class="input_V" type="text" id="coordinacion_' + indiceProducto + '" name="coordinacion_' + indiceProducto + '" placeholder="Coordinación" value="' + coordinacionInicial + '"></td>'; // Coordinación
    htmlProducto += '<td><input class="input_V" type="text" id="recibio_' + indiceProducto + '" name="recibio_' + indiceProducto + '" placeholder="Recibió" value="' + recibioInicial + '"></td>'; // Recibió
    htmlProducto += '<td><input type="file" id="evidencia_' + indiceProducto + '" name="evidencia_' + indiceProducto + '" accept="image/*" value="' + evidenciaInicial + '"></td>'; // Evidencia
...
function actualizarNumProductos() {
   ...
   function calcularImporte(indice) {
    ...
    let precio = document.getElementById("precio_" + indice).textContent.trim(); // Obtener el precio sin espacios extra
    precio = parseFloat(precio.replace("$", "")); // Eliminar el símbolo de moneda y convertir a número
